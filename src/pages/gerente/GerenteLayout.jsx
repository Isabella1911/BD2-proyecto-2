...
import {
  ...
  actualizarAsignadoA,
  eliminarAsignadoA,
  eliminarAsignadoABulk,

  crearSupervisa,
  actualizarSupervisa,
  eliminarSupervisa,
  eliminarSupervisaBulk,
  actualizarSupervisaPorNombres,
  eliminarSupervisaBulkPorNombres,

  crearTieneInventario,
  actualizarTieneInventario,
  eliminarTieneInventario,
  eliminarTieneInventarioBulk,

  crearAlmacenadoEn,
  actualizarAlmacenadoEn,
  eliminarAlmacenadoEn,
  eliminarAlmacenadoEnBulk,
...
const REL_CONFIG = {
  ...
  SUPERVISA: {
    ...
    actualizarBulkPorNombres: (nombres, p) =>
      actualizarSupervisaPorNombres(nombres, p),

    eliminarBulkPorNombres: (nombres) =>
      eliminarSupervisaBulkPorNombres(nombres),

    bulkUsaNombres: true,
    bulkLabel: "Nombres de supermercados",
    bulkPlaceholder: "Supermercado Central, Supermercado Norte, Supermercado Sur...",
...
export default function GestionRelaciones() {
  ...
  const handleCrear = async () => {
    ...
    try {
      ...
      cfg.campos.forEach((c) => {
        if (c.type === "boolean") {
          body[c.key] = body[c.key] === "true" || body[c.key] === true;
        }
  ...
  const handleBulk = async () => {
    ...
    if (valores.length === 0) {
      return flash(
        cfg.bulkUsaNombres
          ? "Ingresa al menos un nombre de supermercado"
          : "Ingresa al menos un ID",
        false
      );
    }
    ...
    try {
      if (bulkAccion === "actualizar") {
        if (!bulkPropKey) {
          setLoading(false);
          return flash("Selecciona la propiedad a actualizar", false);
        }

        if (cfg.bulkUsaNombres && cfg.actualizarBulkPorNombres) {
          await cfg.actualizarBulkPorNombres(valores, {
            [bulkPropKey]: bulkPropVal,
          });

          flash(
            `✓ Propiedad "${bulkPropKey}" actualizada en SUPERVISA usando nombres de supermercados`
          );
        } else if (cfg.actualizar) {
          await cfg.actualizar(valores, {
            [bulkPropKey]: bulkPropVal,
          });

          flash(
            `✓ Propiedad "${bulkPropKey}" actualizada en relaciones que incluyen esos IDs`
          );
        }
      } else if (bulkAccion === "eliminar") {
        const texto = cfg.bulkUsaNombres
          ? `${valores.length} supermercados por nombre`
          : `${valores.length} IDs`;

        if (!confirm(`¿Eliminar relaciones con ${texto}?`)) {
          setLoading(false);
          return;
        }

        if (cfg.bulkUsaNombres && cfg.eliminarBulkPorNombres) {
          await cfg.eliminarBulkPorNombres(valores);
        } else if (cfg.eliminarBulk) {
          await cfg.eliminarBulk(valores);
        }
  ...
  return (
    <div className="admin-nodos">
      ...
      <p className="an-hint" style={{ marginTop: 6 }}>
        {cfg.descripcion}
      </p>
      ...
      {subTab === "editar" && (
        <div className="an-panel">
          ...
          {!cfg.actualizar ? (
            <p className="an-hint">
              Esta relación no soporta edición directa de propiedades.
            </p>
          ) : (
            <>
              <p className="an-hint">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const keys = cfg.campos
                      .filter((c) => !c.source)
                      .map((c) => c.key);

                    const idKeys = cfg.campos
                      .filter((c) => c.source)
                      .map((c) => c.key);
      ...
      {subTab === "eliminar" && (
        <div className="an-panel">
          ...
          {!cfg.eliminar ? (
            <p className="an-hint">
              Esta relación no soporta eliminación directa desde aquí.
            </p>
          ) : (
            <>
              <p className="an-hint">
                Selecciona los nodos extremos para identificar la relación a eliminar.
              </p>
              ...
              <button
                className="an-btn-primary an-btn-danger"
      ...
      {subTab === "bulk" && (
        <div className="an-panel">
          ...
          <p className="an-hint">
            {cfg.bulkUsaNombres
              ? "Ingresa nombres de supermercados separados por coma. Esta opción aplica para SUPERVISA."
              : "Ingresa IDs separados por coma. Para ASIGNADO_A, TIENE_INVENTARIO y otras relaciones, los IDs son de los nodos destino."}
          </p>

          <div className="an-field">
            <label>{cfg.bulkLabel ?? "IDs"} separados por coma</label>

            <input
              type="text"
              placeholder={cfg.bulkPlaceholder ?? "id1, id2, id3..."}
              value={bulkIds}
          ...
          <div className="an-field">
            ...
            >
              ...
              {cfg.eliminarBulk && (
                <option value="eliminar">Eliminar relaciones</option>
              )}
            </select>
          </div>
          ...
          <button
            className={`an-btn-primary ${
              bulkAccion === "eliminar" ? "an-btn-danger" : ""
            }`}
            onClick={handleBulk}
            disabled={loading}
          >
